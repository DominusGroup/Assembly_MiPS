...
;------Pasa las instrucciones en el .txt al Buffer  
...
;-----------------------------------------------------------------------------------------
;---- Etiqueta encargada de Imprimir en pantalla las decodificaciones para instrucciones R
;-----------------------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------------------  
;---- Etiqueta encargada de Imprimir en pantalla las decodificaciones para instrucciones I
;-----------------------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------------------
;---- Etiqueta encargada de Imprimir en pantalla las decodificaciones para instrucciones J
;-----------------------------------------------------------------------------------------
	  	_PantallaTipoJ:	  	
 		impr_textoPantalla enterNow, 1
 		impr_textoPantalla enterNow, 1 		
...
;-----------------------------------------------------------------------------------------
;-------- Macro que pasa el dato de Ascii a Hex 
;-----------------------------------------------------------------------------------------

;-------------------------  MACRO   ----------------------------------
;Macro: Gets 8-bytes, from index into buffer.
...
;-----------------------------------------------------------------------------------------
;----------- Etiqueta encargada de almacenar las Instrucciones en memoria 
;-----------------------------------------------------------------------------------------
_Instruction2Memory: 
...
;------------------------------------------------------------------------------------------------
; Etiqueta encargada de pasar dato de salida a Ascii para desplegar en pantalla o guardar en .txt
;------------------------------------------------------------------------------------------------
Hex2Ascii:
	xor rax,      rax
	mov rcx , rax
...
;			ret 
	...
	_imul: ; *******
		...
		imul r9 
		_continueImul:
			mov rsi, rax    ; Registro de 64 bits que contiene resultado de multiplicación 
...
section .data
;-------------------  Memory Data -------------------------
  iMEM_BYTES:     equ 400           ; x/4 = words num	; Memoria de instrucciones  ( 100 palabras )
  REG_BYTES:	  equ 128              				    ; Banco de registros 		( 32 palabras )
  DATAMEM_BYTES:  equ 400                               ; Memoria de datos          ( 100 palabras )
  DecodeProccess: equ 320  
  ...
  FILE_NAME:      db "ROM2.txt", 0
  FILE_LENGTH:    equ  701				        		; length text inside 
...
_MIPS:
	...
	xor rcx, 			rcx 
	xor r12, r12
	sub rsp, 			TOT_MEM         ; Memory bytes allocation		
...
;----------------- Initialization of MiPS Arguments Registers --------------------
	call _ArgumentsRegInit

 
_Reg001:
	mov rcx, iMEM_BYTES/4   							; Loop counter  
...
;--------------------------------------------------------
;---------- Loop encargado de procesar instrucciones 
;--------------------------------------------------------
_PCLoop:
